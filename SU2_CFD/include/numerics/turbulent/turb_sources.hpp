...
struct CSAVariables {
  ...
  su2double ft2, d_ft2, r, d_r, g, d_g, glim, fw, d_fw, Ji, d_Ji, Shat, d_Shat, fv1, d_fv1, fv2, d_fv2, Prod, density;
...
class CSourcePieceWise_TurbWA : public CNumerics {
 ...
 public:
  ...
  ResidualType<> ComputeResidual(const CConfig* config) override {
    ...
    su2double S_y, VP_StrainMag_Grad2_i, VP_GradR_dot_GradS_i, VP_ScalarVar_Grad2_i;
    if (waParsedOptions.version == WA_OPTIONS::VP) {
      ...
      VP_GradR_dot_GradS_i = GeometryToolbox::DotProduct(nDim, AuxVar_Grad_i[2], AuxVar_Grad_i[1]);
      VP_ScalarVar_Grad2_i = GeometryToolbox::SquaredNorm(nDim, AuxVar_Grad_i[2]);
    }
    ...
    if (dist_i > 1e-10) {
      ...
      su2double Dest1;
      if(waParsedOptions.version == WA_OPTIONS::VP) {
        Dest1 = f1_i * C_2kom * ScalarVar_i[0] / StrainMag_i * pow(S_y, 2.0) / pow(Laminar_Viscosity_i, 2.0) * VP_GradR_dot_GradS_i;
      } else {
      ...
      if (waParsedOptions.version == WA_OPTIONS::VP) {
        Jacobian_i[0] += (f1_i * C_2kom * pow(S_y, 2.0) / pow(Laminar_Viscosity_i, 2.0) / StrainMag_i * VP_GradR_dot_GradS_i)*Volume;
      } else {
